const events = {
    attachFormEvents: function() {
        document.getElementById("condicionesForm").addEventListener("submit", function(event) {
            ...
            if (!events.validateVelocidad()) { event.preventDefault(); return; }
    ...
    validateVelocidad: function() {
        const velMedidaInput = document.getElementById("velMedida");
        if (velMedidaInput && (parseFloat(velMedidaInput.value) <= 0 || isNaN(parseFloat(velMedidaInput.value)))) {
            ui.showError("Ingrese la Velocidad Medida.");
            velMedidaInput.focus();
            return false;
        }
        return true;
    },
